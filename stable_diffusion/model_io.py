...
def load_unet(
    key: str = _DEFAULT_MODEL, float16: bool = False, file_path: Path = None
):
    ...
    if file_path:
        with open(file_path / unet_config) as f:
            config = json.load(f)
    else:
    ...
    if file_path:
        weight_file = str(file_path / unet_weights)
    else:
...
def load_text_encoder(
    ...
    if file_path:
        with open(file_path / text_encoder_config) as f:
            config = json.load(f)
    else:
    ...
    model = CLIPTextModel(
        CLIPTextModelConfig(
            ...
            projection_dim=(
                config["projection_dim"] if with_projection else None
            ),
    ...
    if file_path:
        weight_file = str(file_path / text_encoder_weights)
    else:
        weight_file = hf_hub_download(key, text_encoder_weights)
    _load_safetensor_weights(
        map_clip_text_encoder_weights, model, weight_file, float16
    )
...
def load_autoencoder(
    ...
    if file_path:
        with open(file_path / vae_config) as f:
            config = json.load(f)
    else:
    ...
    if file_path:
        weight_file = str(file_path / vae_weights)
    else:
...
def load_diffusion_config(key: str = _DEFAULT_MODEL, file_path: Path = None):
    ...
    if file_path:
        with open(file_path / diffusion_config) as f:
            config = json.load(f)
    else:
...
def load_tokenizer(
    ...
    if file_path:
        vocab_file = file_path / _MODELS[key][vocab_key]
        with open(vocab_file, encoding="utf-8") as f:
            vocab = json.load(f)

        merges_file = file_path / _MODELS[key][merges_key]
