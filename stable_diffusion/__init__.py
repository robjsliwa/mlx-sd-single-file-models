...
class StableDiffusion:
    def __init__(
        ...
        self.diffusion_config = load_diffusion_config(
            model, file_path=file_path
        )
        self.unet = load_unet(model, float16, file_path=file_path)
        self.text_encoder = load_text_encoder(
            model, float16, file_path=file_path
        )
        self.autoencoder = load_autoencoder(model, False, file_path=file_path)
        self.sampler = SimpleEulerSampler(self.diffusion_config)
        self.tokenizer = load_tokenizer(model, file_path=file_path)

    @staticmethod
    def from_single_file(
        file_path: Path, model: str = _DEFAULT_MODEL, float16: bool = False
    ):
        return StableDiffusion(model, float16, file_path)
...
class StableDiffusionXL(StableDiffusion):
    ...
    def _get_text_conditioning(
        ...
        if n_images > 1:
            conditioning = mx.repeat(conditioning, n_images, axis=0)
            pooled_conditioning = mx.repeat(
                pooled_conditioning, n_images, axis=0
            )
